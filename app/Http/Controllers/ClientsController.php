...
class ClientsController extends Controller {
	...
	public function clientInit(Request $request){
		...
		$client = DB::table('customers')->select('customers.*', 'customer_documents.aadhaar_card', 'customer_documents.pan_card', 'customer_documents.voter_id_card', 'customer_documents.customer_photo', 'customer_documents.bank_passbook','customer_documents.joint_photo', 'banks.bank_name')->leftjoin('customer_documents', 'customer_documents.customer_id', '=', 'customers.id')->leftjoin('banks', 'banks.id', '=', 'customers.bank_id')->where('customers.id', $request->client_id)->first();

		if($client){
			$client->aadhaar_no = intval($client->aadhaar_no);
			$client->pin = intval($client->pin);
			$client->ac_no = intval($client->ac_no);
			$client->mobile = intval($client->mobile);


			$guarantor = DB::table('customer_guarantor')->select('customer_guarantor.*', 'banks.bank_name')->leftjoin('banks', 'banks.id', '=', 'customer_guarantor.bank_id')->where('customer_guarantor.customer_id', $client->id)->first();
	...
	public function clientDetailsInit(Request $request){
		...
		$client = DB::table('customers')->select('customers.*', 'customer_documents.aadhaar_card', 'customer_documents.pan_card', 'customer_documents.voter_id_card', 'customer_documents.customer_photo','customer_documents.bank_passbook','customer_documents.joint_photo', 'states.state_name', 'cities.city_name as district_name', 'blocks.block_name', 'villages.village_name','banks.bank_name')
		->leftjoin('customer_documents', 'customer_documents.customer_id', '=', 'customers.id')
		...
		$guarantorData = DB::table('customer_guarantor')->select("customer_guarantor.*", 'banks.bank_name')->leftjoin('banks', 'banks.id', '=', 'customer_guarantor.bank_id')->where('customer_id', $client->id)->first();
		$client->dob = $client->dob ? date('d-m-Y', strtotime($client->dob)) : '';
