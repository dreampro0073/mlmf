...
class Plan extends Authenticatable {
    ...
    public static function collection ($today_groups){
        ...
        foreach ($today_groups as $key => $item) {
            $customer_count = DB::table('group_customers')->where('group_id',$item->group_id)->count();
            $today_target = $today_target + ($customer_count*$item->emi_amount);

            $emi_collection_count = DB::table('emi_collection')->where('group_id',$item->group_id)->where('group_emi_date_id',$item->id)->where('collected_amount',1)->count();
